...
$enlace = mysql_connect("localhost", "root", "root123");
mysql_select_db('empleo', $enlace);
if ($enlace) {
    $msg = "exito...";
} else {
...
function mostrarDatos($form) {
    ...
    for ($i = 0; $i < $nroCol; $i++) {
        $nomFila = mysql_field_name($rs, $i);
        $longitud = mysql_field_len($rs, $i);
        $tipoDato = mysql_field_type($rs, $i);
