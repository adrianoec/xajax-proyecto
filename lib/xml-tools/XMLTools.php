...
//Initializations 
    ...
    private function _libxml_display_error($error)
    {
        ...
        $errLine = str_pad($error->line, 3, "0", STR_PAD_LEFT);
    ...
    /**
     * Metodo para  validar la estructura del xml version: xsd_1.1.1
     * 
     * @param type $xml
     * @param type $document factura|notacredito|notadebito|guiaremision|comprobanteretencion
     * return boolean
     */
    public function Validate($xml, $document)
    {
        $document = trim(strtolower($document));
        switch ($document) {
            case "factura":
                $resp = $this->ValidateXML($xml, "/home/ecawi/lib/xml-tools/xsd_1.1.0/factura.xsd");
                break;
            case "notacredito":
                $resp = $this->ValidateXML($xml, "/home/ecawi/lib/xml-tools/xsd_1.1.1/notaCredito.xsd");
                break;
            case "notadebito":
                $resp = $this->ValidateXML($xml, "/home/ecawi/lib/xml-tools/xsd_1.1.1/notaDebito.xsd");
                break;
            case "guiaremision":
                $resp = $this->ValidateXML($xml, "/home/ecawi/lib/xml-tools/xsd_1.1.1/guiaRemision.xsd");
                break;
            case "comprobanteretencion":
                $resp = $this->ValidateXML($xml, "/home/ecawi/lib/xml-tools/xsd_1.1.1/comprobanteRetencion.xsd");
                break;

            default:
                $resp = array("xsd del documento ($document) no encontrado");
                $data = null;
                break;
        }
        ...
        return array("valid"=>$valid,"data"=>$data);
    }
