    ...
    function loadTabla(){$("table").tablesorter({ widgets: ['zebra']});  }  $(function() {$("table") .tablesorter({ widgets: ['zebra']});  });
    ...
    function muestra_disabled(id){
        if (document.getElementById){ //se obtiene el id
            var el = document.getElementById(id); //se define la variable "el" igual a nuestro div
            el.style.display = (el.disabled == 'disabled') ? '' : 'disabled'; //damos un atributo display:none que oculta el div
        }
    }
